#!/usr/bin/env bash
# Load test for CAS:pok_normplusklimaatverandering2100_50cm layer
...
gum style \
  ...
  "⚡ Load Test: Climate Change Layer" \
  "" \
  "Climate Impact 2100 (50cm sea level rise)" \
  "5000 requests • 100 concurrent connections"

echo ""

# Tile URL for Netherlands area
TILE_URL="https://climate-adaptation-services.geospatialhosting.com/geoserver/gwc/service/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=CAS:pok_normplusklimaatverandering2100_50cm&STYLE=&TILEMATRIXSET=WebMercatorQuad&TILEMATRIX=8&TILEROW=84&TILECOL=133&FORMAT=image/png"
...
gum style --foreground "$KARTOZA_HIGHLIGHT3" "   • Results will be saved to pok_normplusklimaatverandering2100_50cm_results.csv"
gum style --foreground "$KARTOZA_HIGHLIGHT3" "   • Gnuplot data: pok_normplusklimaatverandering2100_50cm_load.data"
...
ab -n 5000 -c 100 \
  -g pok_normplusklimaatverandering2100_50cm_load.data \
  -e pok_normplusklimaatverandering2100_50cm_results.csv \
  -H "Accept: image/png,*/*" \
...
if [ $? -eq 0 ] && [ -f "pok_normplusklimaatverandering2100_50cm_results.csv" ]; then
  gum style \
    --foreground 118 --border-foreground 118 --border double \
    --align center --width 60 --margin "1 2" --padding "1 2" \
    "✅ Load Test Completed Successfully!" \
    "" \
    "📊 Results: pok_normplusklimaatverandering2100_50cm_results.csv" \
    "📈 Gnuplot data: pok_normplusklimaatverandering2100_50cm_load.data"

  # Show quick summary if CSV exists
  if [ -f "pok_normplusklimaatverandering2100_50cm_results.csv" ] && [ -s "pok_normplusklimaatverandering2100_50cm_results.csv" ]; then
    echo ""
    gum style --foreground "$KARTOZA_HIGHLIGHT2" "📈 Quick Performance Summary:"
    tail -1 "pok_normplusklimaatverandering2100_50cm_results.csv" | awk -F, '{
