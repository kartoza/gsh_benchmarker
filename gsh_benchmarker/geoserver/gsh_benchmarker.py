...
def show_help():
    ...
    help_text.append("\n\n")
    help_text.append("Available Commands:", style=f"bold {KARTOZA_COLORS['highlight1']}")
    help_text.append("\n")
    help_text.append("  --help              Show this help message\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  --connectivity      Test connectivity to all layers\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  --single LAYER      Run test for specific layer\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  --comprehensive     Run comprehensive test suite\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  --results           Show summary of recent results\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("\n")
    help_text.append("Options:", style=f"bold {KARTOZA_COLORS['highlight1']}")
    help_text.append("\n")
    help_text.append("  --requests N        Number of requests per test (default: 5000)\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  --concurrency LIST  Comma-separated concurrency levels\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("                      (default: 1,10,100,500,1000,2000,3000,4000,5000)\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("\n")
    help_text.append("Layer Discovery:", style=f"bold {KARTOZA_COLORS['highlight1']}")
    help_text.append("\n")
    help_text.append("  Layers are discovered dynamically from GeoServer GetCapabilities\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("  No hardcoded layer list - works with any GeoServer instance\n", style=f"{KARTOZA_COLORS['highlight3']}")
    help_text.append("\n")
    ...
    help_text.append("  python3 geotest.py --connectivity\n", style=f"{KARTOZA_COLORS['highlight4']}")
    help_text.append("  python3 geotest.py --single AfstandTotKoelte --requests 1000\n", style=f"{KARTOZA_COLORS['highlight4']}")
    help_text.append("  python3 geotest.py --comprehensive --concurrency 1,10,100\n", style=f"{KARTOZA_COLORS['highlight4']}")
...
def run_single_test(layer_name: str, requests: int, concurrency: int, server_url=None):
    ...
    if not layer_info:
        ...
        console.print(f"[{KARTOZA_COLORS['highlight3']}]Available layers: {', '.join(tester.get_layer_list())}[/]")
        return
    ...
    if not is_accessible:
        console.print(f"[{KARTOZA_COLORS['alert']}]❌ Layer not accessible (HTTP {status_code})[/]")
        return
    ...
    if result:
        ...
        console.print()
        console.print(f"[{KARTOZA_COLORS['highlight3']}]Results saved to: {tester.results_dir}/{result.test_id}.*[/]")
    else:
        console.print(f"[{KARTOZA_COLORS['alert']}]❌ Test failed[/]")

def run_comprehensive_test(requests: int, concurrency_levels: List[int], server_url=None):
    ...
    console.print(f"[{KARTOZA_COLORS['highlight2']}]🔥 Running comprehensive load test suite[/]")
    console.print()
    ...
    if results:
        ...
        console.print(table)
        console.print()
        console.print(f"[{KARTOZA_COLORS['highlight3']}]Results saved to: {tester.results_dir}/[/]")
        console.print(f"[{KARTOZA_COLORS['highlight3']}]Generate PDF report with: python3 benchmark_report_generator.py[/]")
    else:
...
Examples:
  ...
  python3 geotest.py --single AfstandTotKoelte # Test single layer
  python3 geotest.py --comprehensive           # Run all tests
        """
    )
    ...
    parser.add_argument('--connectivity', action='store_true',
                       help='Test connectivity to all layers')
    parser.add_argument('--single', metavar='LAYER',
