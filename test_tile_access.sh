...
gum style --foreground 240 "🌐 Base URL: $WMTS_BASE"
gum style --foreground 240 "📍 Tile coordinates: Col=$TILECOL, Row=$TILEROW, Zoom=$ZOOM"
echo ""
...
for layer in "${!layers[@]}"; do
    ...
    gum style --foreground 212 "🧪 Testing: $description"
    gum style --foreground 240 "   Layer: $layer"
    ...
    if echo "$response" | grep -q "200 OK"; then
        gum style --foreground 118 "   ✅ SUCCESS - HTTP 200 OK"
        ((success_count++))
    elif echo "$response" | grep -q "HTTP"; then
        status=$(echo "$response" | awk '{print $2}')
        gum style --foreground 196 "   ❌ FAILED - HTTP $status"
    else
        gum style --foreground 196 "   ❌ FAILED - No response"
    fi
    
    # Show content type if available
    content_type=$(curl -s -I "$TILE_URL" | grep -i "content-type" | cut -d: -f2 | tr -d ' \r')
    if [ -n "$content_type" ]; then
        gum style --foreground 240 "   📄 Content-Type: $content_type"
    fi
...
echo ""
gum style --foreground 212 "📋 Sample tile URLs for reference:"
for i in "${!urls[@]}"; do
    layer_num=$((i + 1))
    url="${urls[$i]}"
    gum style --foreground 240 "   $layer_num. ${url:0:80}..."
done