...
preview_layer() {
  ...
  gum style "🖼️  Fetching preview for: $description"
  ...
  if [ -f "$preview_file" ] && [ -s "$preview_file" ]; then
    gum style "Preview for $description:"
    echo ""
    chafa --size 80x25 --format symbols "$preview_file"
    echo ""
    gum style "Image saved to: $preview_file"
    echo ""
    gum confirm "Continue?" || return
  else
    gum style "❌ Failed to fetch preview for $layer_name"
...
run_load_test() {
  ...
  # Get test parameters
  gum style "⚡ Configuring load test for: $description"
  ...
  gum confirm "$(gum style "⚠️  Warning: This will send $requests requests with $concurrency concurrent connections to the server. Continue?")" || return
  ...
  gum style "🚀 Starting load test..."
  gum style "Target: $tile_url"
  echo ""
  ...
  if [ $? -eq 0 ]; then
    gum style "✅ Connectivity test passed"
  else
    gum style "❌ Connectivity test failed"
    return 1
  fi

  # Warm-up test
  echo ""
  gum style "🔥 Running warm-up (100 requests, 10 concurrent)..."
  ...
  # Main load test
  echo ""
  gum style "⚡ Running main load test..."
  gum style "Progress will be shown below..."
  echo ""
  ...
  echo ""
  gum style "✅ Load test completed!"
  gum style "Results saved to: ${result_prefix}.*"

  # Show summary
  if [ -f "${result_prefix}.csv" ]; then
    echo ""
    gum style "📊 Performance Summary:"
...
generate_pdf_report() {
  gum style "$KARTOZA_HIGHLIGHT2" --border thick \
  ...
  if [[ -n "$latest_report" ]]; then
    ...
    gum style "$KARTOZA_ALERT" "❌ Failed to generate PDF report"
  fi
}

run_all_tests() {
  gum style "🔥 Running ALL load tests in sequence"
  ...
  gum confirm "$(gum style "⚠️  This will run ${#LAYERS[@]} tests with $total_requests total requests. Continue?")" || return
  ...
  for layer_key in "${!LAYERS[@]}"; do
    ...
    gum style --border double --padding 1 "Testing: $description"
    ...
    gum style "✅ Completed: $description"
  ...
  gum style "🎉 All tests completed!"
  gum style "Results saved in results/ directory"
...
view_results() {
  ...
  if [ ! -d "$results_dir" ] || [ -z "$(ls -A $results_dir 2>/dev/null)" ]; then
    gum style "❌ No results found. Run some tests first!"
    return
  fi

  gum style "📊 Available Results:"
  ...
  if [ -n "$selected_file" ]; then
    gum style "📖 Viewing: $(basename $selected_file)"
    echo ""
...
main_menu() {
  while true; do
    ...
    "🔧 Test Connectivity")
      gum style "🔍 Testing connectivity to all layers..."
      for layer_key in "${!LAYERS[@]}"; do
        local layer_name=${LAYERS[$layer_key]}
        local description=${DESCRIPTIONS[$layer_key]}
        local tile_url=$(get_tile_url "$layer_name")
