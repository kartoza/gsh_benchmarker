#!/usr/bin/env bash
# Load test for CAS:AfstandTotKoelte layer
...
gum style \
  ...
  "⚡ Load Test: AfstandTotKoelte Layer" \
  "" \
...
TILE_URL="https://climate-adaptation-services.geospatialhosting.com/geoserver/gwc/service/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=CAS:AfstandTotKoelte&STYLE=&TILEMATRIXSET=WebMercatorQuad&TILEMATRIX=8&TILEROW=84&TILECOL=133&FORMAT=image/png"
...
gum style --foreground "$KARTOZA_HIGHLIGHT3" "   • Results will be saved to AfstandTotKoelte_results.csv"
gum style --foreground "$KARTOZA_HIGHLIGHT3" "   • Gnuplot data: AfstandTotKoelte_load.data"
...
ab -n 5000 -c 100 \
  -g AfstandTotKoelte_load.data \
  -e AfstandTotKoelte_results.csv \
  -H "Accept: image/png,*/*" \
...
if [ $? -eq 0 ] && [ -f "AfstandTotKoelte_results.csv" ]; then
  gum style \
    --foreground 118 --border-foreground 118 --border double \
    --align center --width 60 --margin "1 2" --padding "1 2" \
    "✅ Load Test Completed Successfully!" \
    "" \
    "📊 Results: AfstandTotKoelte_results.csv" \
    "📈 Gnuplot data: AfstandTotKoelte_load.data"

  # Show quick summary if CSV exists
  if [ -f "AfstandTotKoelte_results.csv" ] && [ -s "AfstandTotKoelte_results.csv" ]; then
    echo ""
    gum style --foreground "$KARTOZA_HIGHLIGHT2" "📈 Quick Performance Summary:"
    tail -1 "AfstandTotKoelte_results.csv" | awk -F, '{
